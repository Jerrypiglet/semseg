...
class PSPNet(nn.Module):
    def __init__(self, layers=50, bins=(1, 2, 3, 6), dropout=0.1, classes=2, zoom_factor=8, use_ppm=True, criterion=nn.CrossEntropyLoss(ignore_index=255), pretrained=True, args=None):
        ...
        if layers == 50:
            resnet = models.resnet50(pretrained=pretrained, args=args)
        elif layers == 101:
            resnet = models.resnet101(pretrained=pretrained, args=args)
        else:
            resnet = models.resnet152(pretrained=pretrained, args=args)
    ...
    def forward(self, x, y=None):
        x_size = x.size()
        assert (x_size[2]-1) % 8 == 0 and (x_size[3]-1) % 8 == 0, 'x_size[2] %d, x_size[3] %d'%(x_size[2], x_size[3])
